...
import spark.jobserver.api.{ContextLike, NamedRddSupport, SparkJobInvalid, SparkJobValid}
...
class JobManagerActor(dao: JobDAO,
  ...
  private def getJobFuture(jobJarInfo: JobJarInfo,
    ...
    Future {
      ...
      val job = constructor()
      job match {
        case support: NamedRddSupport =>
          val namedRdds = support.namedRddsPrivate
