...
object ContextSupervisor {
  ...
  // Messages/actions
  case object AddContextsFromConfig

  // Start up initial contexts
  ...
  case class GetContext(name: String)

  // returns JobManager, JobResultActor
  case class GetResultActor(name: String)

  // returns JobResultActor
...
class LocalContextSupervisorActor(dao: JobDAO) extends InstrumentedActor {
  ...
  val contextTimeout = SparkJobUtils.getContextTimeout(config) // to get ExecutionContext for futures
