 ...
 *
 * Note that to take advantage of NamedRddSupport, a job must mix this in and use the APIs here
 * instead of the native RDD `cache()`, otherwise we will not know about the names.
 */
trait NamedRdds {
  // Default timeout is 60 seconds.
  // Hopefully that is enough to let most RDD generator functions finish.
   ...
   *
   * If the given RDD has already been computed by another job and cached in memory, this method
   * will return a reference to the cached RDD. If the RDD has never been computed,
   * then the generator will be called to compute it, in the caller's thread,
   * and the result will be cached and returned to the caller.
   *
   * If an RDD is requested by thread B while thread A is generating the RDD, thread B
   * will block up to the duration specified by @timeout. If thread A finishes generating the RDD
   * within that time, then thread B will get a reference to the newly-created RDD. If thread A does
   * not finish generating the RDD within that time, then thread B will throw a timeout exception.
   *
   ...
   *
   * Note that a previously-known RDD could 'disappear' if it hasn't been used for a while,
   * because the SparkContext garbage-collects old cached RDDs.
   *
   * @param name the unique name of the RDD. The uniqueness is scoped to the current SparkContext.
   * @param timeout if the RddManager doesn't respond within this timeout, an error will be thrown.
   * @tparam T the generic type of the RDD.
  ...
  /**
   * Replaces an existing RDD with a given name with a new RDD. If an old RDD for the given name
   * existed, it is un-persisted (non-blocking) and destroyed. It is safe to call this method when
   * there is no existing RDD with the given name. If multiple threads call this around the same
   * time, the end result is undefined - one of the generated RDDs will win and will be returned
   * from future calls to get().
   *
   * The rdd generator function will be called from the caller's thread. Note that if this is
   * called at the same time as getOrElseCreate() for the same name, and completes before the
   * getOrElseCreate() call, then threads waiting for the result of getOrElseCreate() will unblock
   * with the result of this update() call. When the getOrElseCreate() succeeds, it will replace the
   * result of this update() call.
   *
   * @param name the unique name of the RDD. The uniqueness is scoped to the current SparkContext.
   ...
   * Has no effect if no RDD with this name exists.
   *
   * @param name the unique name of the RDD. The uniqueness is scoped to the current SparkContext.
   */
   ...
   *
   * Note: this returns a snapshot of RDD names at one point in time.
