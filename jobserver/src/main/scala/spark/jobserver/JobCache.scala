...
/**
 * A cache for SparkJob classes.  A lot of times jobs are run repeatedly, and especially
 * for low-latency jobs, why retrieve the jar and load it every single time?
 */
class JobCache(
  maxEntries: Int,
  dao: JobDAO,
